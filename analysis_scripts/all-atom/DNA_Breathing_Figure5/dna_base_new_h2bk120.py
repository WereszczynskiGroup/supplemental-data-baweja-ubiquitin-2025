...
for i in range(1, 4):
    ...
    H3_1_2 = "resid 287:315 or resid 652:680"  # H3 residues for nucleosome axis
    DNA_resid = "resid 1206 or resid 1353"    # DNA residues for axis definition
    
    # DNA base pair residue ranges for both strands
    strand1_residues = list(range(1133, 1206))  # First strand
    strand2_residues = list(range(1426, 1353, -1))  # Complementary strand (reverse order)
    strand3_residues = list(range(1279, 1206,-1))  # First strand
    strand4_residues = list(range(1280, 1353))  # Complementary strand (reverse order)
    ...
    nucleosome_point = u.atoms.select_atoms(f"resid 287:315").center_of_mass()  # A point on the nucleosome axis
    
    with open('dimer1_h2b.unwrap_'+'side1_sim'+str(i), 'w') as f:
    ...
    with open('dimer1_h2b.unwrap_'+'side2_sim'+str(i), 'w') as f:
       for ts in u.trajectory[::100]:
           wrapped_bp_count = 0
           total_bp = len(strand3_residues)
