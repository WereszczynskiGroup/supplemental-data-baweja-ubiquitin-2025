...
# check maximum number of H-bonds on a single phosphate
#hbpps=[] # H-bond per phosphate array
#for bset in bsets:
#  hbpps.append({})
#  for b in bset: hbpps[-1][(b[3],b[4])]=[0,[]]
#for bset,hbpp in zip(bsets,hbpps):
#  for b in bset:
#    hbpp[(b[3],b[4])][0]+=1
#    hbpp[(b[3],b[4])][1].append(b)
#for bset,hbpp in zip(bsets,hbpps):
#  for phos in hbpp: print phos,hbpp[phos]
...
for bset,u in zip(bsets,uarray):
  ...
  for b in bset:
    ...
    if(ca.serial not in pdns.keys()): pdns[ca.serial]=[]
    c=Pdns_entry()
    c.chain=chain2num[ca.segid]
    c.serial=ca.serial
    c.serial2=ca.serial-ridshift[ca.segid]
    c.dist=dist
    c.theta1=theta1
    c.theta2=theta2
    ...
    c.b=b
    pdns[ca.serial].append(c)
# sort
for serial in pdns:
  pdns[serial]=sorted(pdns[serial], key=lambda c: c.b[4]) # sort by acceptor_resid

# check conistency of hbonds list
#bset_debug=Set()
#for serial in pdns:
#  a=uarray[0].atoms[serial-1]
#  bset_debug.add((chain2num[a.segid],a.resname, a.resid))
#bset_debug=sorted(bset_debug, key=lambda b: b[2]) # donor_resid
#bset_debug=sorted(bset_debug, key=lambda b: b[0]) # donor_segid
#for b in bset_debug: print b[0],b[1],b[2]
#sys.exit()

# average 1kx5 and 601, and print out
icontact=0
print ('<<<< pdns')
keylist = pdns.keys()
keylist.sort()
for serial in keylist:
  if(len(pdns[serial])%2==0):
    # average contacts from different native structures, they should be sorted in the proper way
    for i in range(len(pdns[serial])/2):
      c1=pdns[serial][i*2]
      c2=pdns[serial][i*2+1]
      if(c1.native==c2.native): # error, you should average different native structures
        #print '# error: len(pdns[serial])==2 and c1.native==c2.native'; sys.exit()
      ...
      icontact+=1; #print "pdns",icontact,c.chain,c.serial2,c.serial2,dist,theta1,theta2,"-1.0"
  if(len(pdns[serial])%2!=0 or len(pdns[serial])>4): print '# error: len(pdns[serial])%2!=0 or len(pdns[serial])>4'; sys.exit()
print ('>>>>')
